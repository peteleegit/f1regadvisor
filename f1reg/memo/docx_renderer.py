...
def build_memo_docx(ctx: MemoContext) -> bytes:
    ...
    # ------------------------------------------------------------------
    # Bottom Line — two-track verdict
    # ------------------------------------------------------------------
    doc.add_heading("Bottom Line", 1)
    for label, value in [
        ("Legal standing:", ctx.legal_standing or "ASSESSMENT INCOMPLETE"),
        ("Enforcement risk:", ctx.enforcement_risk or "ASSESSMENT INCOMPLETE"),
        ("FIA predictability:", ctx.fia_predictability or "ASSESSMENT INCOMPLETE"),
    ]:
        p = doc.add_paragraph()
        p.add_run(label + " ").bold = True
        p.add_run(value)
    ...
    # ------------------------------------------------------------------
    # FIA Reality Assessment
    # ------------------------------------------------------------------
    po = ctx.precedent_output
    rto = ctx.regulatory_text_output
    has_reality = (po and po.consistency_rating) or (rto and rto.ambiguities)
    if has_reality:
        doc.add_heading("FIA Reality Assessment", 1)
        if po and po.consistency_rating:
            p = doc.add_paragraph()
            p.add_run("Precedent consistency: ").bold = True
            label = po.consistency_rating
            if po.trend_direction and po.trend_direction != "UNCLEAR":
                label += f" — trend: {po.trend_direction}"
            p.add_run(label)
            if po.consistency_analysis:
                _runs(doc.add_paragraph(), po.consistency_analysis)
        if rto and rto.ambiguities:
            doc.add_paragraph().add_run("Regulatory ambiguities — classified by likely FIA intent:").bold = True
